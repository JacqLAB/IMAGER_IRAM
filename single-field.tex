...
%\> MAP\_WEIGHT \> Weighting mode: natural (optimum in  terms  of  sensitivity)  or  \\
%                \> \> robust (usually  lower sidelobes and higher spatial resolution) weighting. \\
%                \> \> Usage: e.g., LET MAP\_WEIGHT NATURAL.\\
...
\> \sicvar{MAP\_UVTAPER} \> [3] Real \>  Array of 3 values controlling the UV taper: major/minor axis \\
                ...
                \> \> \> By default (0,0,0). (Old name \sicvar{UV\_TAPER[3]}). \\
...
\> \sicvar{MAP\_TAPEREXPO} \> Real \>  the taper exponent. Default 2, indicating Gaussian function. \\
                \> \> \> (Old name \sicvar{TAPER\_EXPO}). \\
%\hspace{0.0cm} \= \hspace{4.2cm} \= \kill
\end{tabbing}
\end{description}
...
%The \com{UV\_MAP} behavior is controlled
%through the following sic variables
%\begin{description}
%\item[Gridding kernel] \mbox{}
%  \begin{description}
%  \item[\texttt{CONVOLUTION}] Gridding in the \uv{} plane is done through
%    convolution and this variable is the kind of convolution kernel.
%    Default is 5, meaning spheroidal functions. Unless you are a real
%    expert, you should never change the value of this variable: Spheroidal
%    functions are the right answer.
%  \end{description}
%\item[Data cube setup] \mbox{}
%  \begin{description}
%  \item[\texttt{WCOL}] Number of the used weight column. The default is to
%    compute only one dirty beam for the whole data cube. This default may
%    be incorrect when adding data obtain at different rest frequencies.
%    % *** we should implement the ONEBEAM variable! JP ***
%  \item[\texttt{MCOL}] First and last velocity/frequency channels to map
%    (default value is 0 meaning all the channels).
%  \item[\texttt{MAP\_CELL}] Pixel size in arcsec. Default is 0, meaning
%    between 1/3 and 1/4 of the synthesized beam.
%  \item[\texttt{MAP\_SIZE}] Map size in pixel. Default is 0, meaning a
%    power of two which gives about twice the size of the primary beam size.
%    For efficiency of FFT, it is better to pick a power of two.
%  \item[\texttt{MAP\_RA}] Right Ascension of the phase center. Default is
%    an empty string, meaning do not change the right ascension.
%  \item[\texttt{MAP\_DEC}] Declination of the phase center. Default is an
%    empty string, meaning do not change the declination.
%  \item[\texttt{MAP\_ANGLE}] Map position angle in degree. Default is 0,
%    meaning do not rotate the map.
%  \item[\texttt{UV\_SHIFT}] Logical which toggle the change of phase center
%    and position angle. This may be used to align the map axes with a
%    feature of interest of the source, by rotation of the \uv{}
%    coordinates.
%  \end{description}
%\item[Weighting] \mbox{}
%  \begin{description}
%  \item[\texttt{WEIGHT\_MODE}] Kind of weighting. Possibilities are
%    \texttt{NATURAL} or \texttt{UN} (which stands for \texttt{ROBUST}...).
%    Default is \texttt{NATURAL} weighting to maximize the point source
%    sensitivity.
%  \item[\texttt{UV\_CELL}] Size of the \uv{} plane cell and threshold of
%    the robust weighting. Default are 7.5~m (half the diameter of the
%    \NOEMA{} antenna) and 1.
%  \end{description}
%\item[Tapering] \mbox{}
%  \begin{description}
%  \item[\texttt{UV\_TAPER}] Properties of the Gaussian used to apodize the
%    weights of the \uv{} visibilities (minor and major axes in meters and
%    position angle in degree). Default is 0, meaning no tapering.
%  \item[\texttt{TAPER\_EXPO}] *** ? JP *** Default is 0, meaning the use of
%    a Gaussian as tapering function.
%  \end{description}
%\end{description}
%The current values of all those parameters may be found through the
%\comm{INPUT}{UV\_MAP} command. The main difficulty in the use of the robust
%weighting and tapering is the choice of the right parameters for
%respectively \texttt{UV\_CELL[2]} and \texttt{UV\_TAPER}. \imager{} offers
%the \com{UV\_STAT} command to guide your choice.
%\begin{verbatim}
%       1 IMAGER> read uv demo.uvt
%       2 IMAGER> uv_stat weight
...
%Comments:
%\begin{description}
%\item[Line 1] Read a \uv{} table in an internal buffer as the
%  \com{UV\_STAT} works only on this buffer.
%\item[Lines 2-14] Displays the synthesized beam (major, minor, position
%  angle), the noise levels (point source in mJy and brightness in K) and
%  the level of the sidelobes as a function of the threshold (1st column) to
%  be used in the robust weighting method. The usual way to choose the right
%  threshold is to select the one that gives a substantially better spatial
%  resolution without increasing the noise by more than 10\%.
%\item[Lines 15-26] Displays the same parameters as above as a function of
%  the tapering distance (1st column) in meter.
%\end{description}
...
Clean convergence is controlled by the usual \sicvar{ARES} (\#1) , \sicvar{FRES} (\#2) 
and \sicvar{NITER} (\#3) criteria,
plus \sicvar{CLEAN\_NCYCLE} for methods with major cycles. However, these criteria
can be set to 0, allowing \imager{} to automatically guess when to stop
using criterium \#4.
In this case, convergence is controlled by \sicvar{CLEAN\_NKEEP}, which is 
...
$N-$\texttt{CLEAN\_NKEEP} for positive signals (resp. negative). In essence, \sicvar{CLEAN\_NKEEP}
...
The main difference between the different basic \clean{} algorithms is the
...
The \com{MX} (cor Cotton-Schwab, from the names of its authors) 
...
In \imager{}, \com{MX} is only implemented for single channel data set.
...
\paragraph{\com{MULTI}}
 ...
 controlled by \sicvar{CLEAN\_SMOOTH}.
...
\item[Support] \mbox{}
  ...
  \item[\com{SUPPORT}] A command that defines the support where
    ...
    stored in the \sicvar{SUPPORT\%} structure. For a Mask, it can be
    read by command \comm{READ}{MASK} or computed by command \comm{SUPPORT}{/THRESHOLD},
    and saved by command \comm{WRITE}{MASK}.
  \end{description}
...
\com{MX} cleans the largest region of the dirty map because the source
removal happens in the \uv{} plane. However, it is slower than \com{CLARK}
because of the repeated imaging step and it shares some of the \com{CLARK}
...
\item[Stopping criterion] Choose the right stopping criterion.\\
  ...
  \item If this empirical noise value is larger than the value computed from the
    ...
    \com{UV\_MAP} command), your observation is probably dynamics limited,
    \ie\ you have a bright source whose dirty sidelobes are much larger
